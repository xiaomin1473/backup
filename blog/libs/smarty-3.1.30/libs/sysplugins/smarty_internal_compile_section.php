...
class Smarty_Internal_Compile_Section extends Smarty_Internal_Compile_Private_ForeachSection
{
     ...
     * @var array
     */
    public $nameProperties = array('first', 'last', 'indexsss', 'iteration', 'show', 'total', 'rownum', 'index_prev',
                                   'index_next', 'loop');
    ...
    public function compile($args, Smarty_Internal_TemplateCompilerBase $compiler)
    {
        ...
        $incFor = array();
        $cmpFor = array();
        $propValue = array('indexsss' => "{$sectionVar}->value['indexsss']", 'show' => 'true', 'step' => 1,
                           'iteration' => "{$local}iteration",

        );
        $propType = array('indexsss' => 2, 'iteration' => 2, 'show' => 0, 'step' => 0,);
        ...
        if ($propType[ 'step' ] == 0) {
            if ($propValue[ 'step' ] == 1) {
                $incFor[ 'indexsss' ] = "{$sectionVar}->value['indexsss']++";
            } elseif ($propValue[ 'step' ] > 1) {
                $incFor[ 'indexsss' ] = "{$sectionVar}->value['indexsss'] += {$propValue['step']}";
            } else {
                $incFor[ 'indexsss' ] = "{$sectionVar}->value['indexsss'] -= " . - $propValue[ 'step' ];
            }
        } else {
        ...
        $initFor[ 'indexsss' ] = "{$sectionVar}->value['indexsss'] = {$propValue['start']}";
        ...
        if (isset($namedAttr[ 'index_prev' ])) {
            $output .= "{$sectionVar}->value['index_prev'] = {$propValue['indexsss']} - {$propValue['step']};\n";
        }
        if (isset($namedAttr[ 'index_next' ])) {
            $output .= "{$sectionVar}->value['index_next'] = {$propValue['indexsss']} + {$propValue['step']};\n";
        }
